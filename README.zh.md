...
免费版功能完整，没有设备数量限制 —— 想接多少音箱、耳机、AirPods 都可以。多设备同时输出、单设备独立音量、长按静音、主题切换、蓝牙耳机电量、开机自启、自动检查更新，统统包含。

### Tutti Pro · 一次买断 $7.99

Pro 解锁一个重度用户专属的便利特性：**硬件音量键接管**。Mac 自带键盘上的音量键（以及支持的外接键盘音量键）会直接控制聚合输出 —— 不用再一台一台调整滑杆。

每次新安装会自动开启 **7 天 Pro 试用**，无需激活码。试用结束后所有基础功能照常使用。

[获取 Tutti Pro 激活码](https://tutti.recents.com/buy) — 一次买断 $7.99，可在 2 台 Mac 上激活。**未来所有 Pro 新功能免费升级**。

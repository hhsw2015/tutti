...
系统自带的图标只能调一个设备的音量。Tutti 直接告诉你正在用哪些设备、各自电量多少、要不要静音，让你把系统图标收起来，腾出菜单栏的位置交给 Tutti。首次启动会引导你完成这一步：系统设置 › 控制中心 › 声音 › 不显示。

## 功能

- **多设备同时输出**：在菜单栏勾选多个输出，自动创建聚合设备（Aggregate Device）并设为系统默认输出。
- **单设备直通**：只选一个时直接切换系统默认输出，不创建多余的虚拟设备。
- **总音量 / 单设备音量 + 静音**：滑杆既能整体推拉，也能单独调每一台；可以单独静音一台，其他继续放。
- **三态状态指示**：区分「全部输出中」「部分静音」「全部静音」，文案与色点同步。
- **音量直控**（Pro）：键盘音量键（F11 / F12 / 静音）控制聚合输出，菜单栏图标或 popover 上的滚轮也能调音量。Shift+Option 细调步长与系统一致。键盘路径需要「辅助功能」权限，滚轮路径不需要。
- **预设**（Pro）：把常用设备组合存为预设，菜单栏一键切换。
- **蓝牙耳机电量**：识别到的蓝牙输出在设备名旁边显示电量。
- **AirPlay 已连接设备识别**：系统已经把音频路由到 AirPlay 接收器后，该设备会作为一个 `airplayaudio` 输出出现在 Tutti 设备列表里，跟普通设备一样可选可切换。
...
Pro 让你不打开 Tutti 也能调音量，把常用设备组合存为预设一键切换：

- **键盘音量键**（F11 / F12 / 静音）直接控制聚合输出。Shift+Option 细调步长与系统一致。
- **鼠标滚轮**在菜单栏图标和 popover 面板上做同一件事。
- **预设**：把常用的设备组合存下来，菜单栏一键切换。
...
- **在 Tutti 里直接切 AirPlay**：目前 Tutti 能识别**已经被系统路由到**的 AirPlay 接收器，把它显示在设备列表里。缺的那一步是「从 Tutti 内部主动切到 HomePod、Apple TV 或别的 AirPlay 接收器」，不用先去控制中心。macOS 目前把 AirPlay 的发现和切换放在只对自家 app 开放的接口里，所以这第一步路由仍然只能在控制中心做。一旦这条路打通，Tutti 第一时间补上。
...
- macOS 13.0 或更高
- 仅在使用 Pro 的键盘音量键接管时，需要授予「辅助功能」权限（滚轮路径不需要）
